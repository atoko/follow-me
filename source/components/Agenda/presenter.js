...
class Agenda extends React.Component {
	...
	render() {
		...
		const mapOnChange = function(args) {
			...
			fetch(`${api.APIROOT}/portals/${Math.floor(coordinates.lat)}/${Math.floor(coordinates.lng)}`, {
				method: "GET",
				mode: "cors"
			}).then((response) => {
				this.setState({
		...
		if (this.state != null && this.state.loading) {
			categories = [
				<div key="modal" className = "loading" style={{overflow:"hidden", height:"50vh"}}>
					<div className="loadingContent"> <button style={{marginLeft:"-40px", marginTop:"-40px", height:"80px", width:"80px"}}  className="button is-loading"> </button></div>											
				</div>];
		}
		var newHeader = <div id="myheader" className="nav">
	<div className="">
	...
	<div className="level nav-right is-hidden-mobile">
		...
		if (portal.center != null) {
			message = <article style={{marginTop: "60px"}} className="message is-primary">
		...
		return (<div className="HolyGrail-body">
				...
				<div className="image" style={{height:"77px", top:"50px", marginTop:"2px", backgroundColor:"teal"}}> 
				</div>
					{categories}
				</div>
			</div>);
