...
function processPortals(portals, center, selected) {
	...
	portals.forEach((p) => {
		...
		var refresh = selected.filter((ps) => {
			return p.id == ps.id;
		});
		if (refresh.length > 0) {
			select.push(p);
...
function filterPortals(state, newPortals) {
	...
	var filtered = portals.filter((p) => {
		return state.filters[p.type_id] == true && p.distance < state.distance;
	});
...
function setPortal(state, action) {
	const { portal } = action;
	var filtered = [];
	var p = state.portals.filter(function(p) {
		return p.id == portal.portal_id;
	});
	if (p.length > 0) {
		var updated = {...p[0] };
		updated.vote = portal;
